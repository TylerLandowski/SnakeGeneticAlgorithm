...
CXPB = 0.0                 # probability that two selected individuals will recombine
GENS = 100  		 	   # number of generations in the run
...
def mutation_replace_end(individual):
	# Get last action
	...
	ind_fit = 4
	if individual.fitness.values: ind_fit = int(individual.fitness.values[0])
	
	to_replace = random.randrange(int(ind_fit / 2), 3 * ind_fit, 1)
